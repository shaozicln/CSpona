      ...
      <div class="comment-list">
        <div v-for="comment in comments" :key="comment.Id" class="comment-item">
          ...
          <div class="comment-main">
            <img
              :src="getImageUrl(comment.User.Avatar || 'boli.jpg')"
              class="avatar"
            />
            <div class="comment-content">
              <div class="comment-header">
                ...
                <span v-if="comment.IsPinned === 1" class="pinned-tag"
                  >置顶</span
                >

                <!-- 三点菜单按钮 - 只有有权限时才显示 -->
                <div v-if="hasCommentActions(comment)" class="comment-actions">
                  <button class="menu-btn" @click.stop="toggleMenu(comment.Id)">
                    ⋮
                  </button>
                  <div v-if="menuStates[comment.Id]" class="action-menu">
                    ...
                    <button
                      v-if="isAdmin"
                      @click="pinComment(comment.Id, true)"
                    >
              ...
              <div class="comment-footer">
                <span class="reply-btn" @click="toggleReply(comment.Id, false)"
                  >回复</span
                >
              </div>
            </div>
          ...
          <!-- 主评论回复框 -->
          <div v-if="activeReplies.parent === comment.Id" class="reply-input">
            <el-input
          ...
          >
            <div
              v-for="(reply, index) in sortedReplies(comment)"
              :key="reply.Id"
              class="reply-item"
            >
              <img
                :src="getImageUrl(reply.User.Avatar || 'boli.jpg')"
                class="avatar"
              />
              <div class="comment-content">
                <div class="comment-header">
                  ...
                  <span v-if="reply.IsPinned === 1" class="pinned-tag"
                    >置顶</span
                  >

                  <!-- 子评论三点菜单按钮 - 只有有权限时才显示 -->
                  <div v-if="hasReplyActions(reply, comment)" class="comment-actions">
                    <button class="menu-btn" @click.stop="toggleMenu(reply.Id)">
                      ⋮
                    </button>
                    <div v-if="menuStates[reply.Id]" class="action-menu">
                      ...
                      <!-- 子评论删除：管理员或父评论本人或本人 -->
                      <button
                        v-if="isAdmin || reply.User.Id == currentUserId || comment.User.Id == currentUserId"
                ...
                <div class="comment-footer">
                  <span class="reply-btn" @click="toggleReply(reply.Id, true)"
                    >回复</span
                  >
                </div>
              </div>
              ...
              <!-- 子评论回复框 -->
              <div v-if="activeReplies.child === reply.Id" class="reply-input">
                <el-input
              ...
              <button @click="toggleShowAllReplies(comment)">
                {{
                  comment.showAllReplies
                    ? "收起回复"
                    : `显示更多回复(${comment.Replies.length - 2}条)`
                }}
              </button>
            </div>
          </div>
...
// 添加菜单状态
const menuStates = ref({}); // 使用对象存储每个评论/回复的菜单状态

const toggleMenu = (commentId) => {
  menuStates.value = {
    ...menuStates.value,
    [commentId]: !menuStates.value[commentId],
  };
};
...
const fetchComments = async () => {
  try {
    const response = await fetch(`${URL}/comments/${articleId}`);
    if (!response.ok) throw new Error("获取评论失败");
    const data = await response.json();

    // 对父评论进行排序：置顶的排前面，然后按时间倒序
